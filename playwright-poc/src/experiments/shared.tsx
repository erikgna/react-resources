...
export function Btn({ onClick, children, danger, disabled }: {
  onClick?: () => void; children: ReactNode; danger?: boolean; disabled?: boolean
}) {
  return (
...
export function Log({ entries }: { entries: string[] }) {
  return (
    <div style={{
      background: '#0a0a0a', border: '1px solid #1e1e1e', borderRadius: 3,
      padding: 10, maxHeight: 160, overflowY: 'auto', marginTop: 10, fontSize: 12,
      ...
      return this.props.fallback
        ...
        : (
          <div style={{ background: '#1a0808', border: '1px solid #5a1111', borderRadius: 4, padding: 14 }}>
            ...
            <button onClick={this.reset} style={{ marginTop: 10, padding: '4px 10px', background: '#2a1111', border: '1px solid #5a1111', color: '#ff6b6b', borderRadius: 3, fontSize: 11 }}>
              Reset
            </button>
          </div>
        )
