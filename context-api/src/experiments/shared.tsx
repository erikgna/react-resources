...
export function Btn({ onClick, children, danger, active }: {
  onClick: () => void; children: ReactNode; danger?: boolean; active?: boolean
}) {
  return (
    <button onClick={onClick} style={{
      padding: '5px 12px',
      background: danger ? '#2a1111' : active ? '#1a2a1a' : '#1e1e1e',
      border: `1px solid ${danger ? '#5a1111' : active ? '#3a6a3a' : '#2a2a2a'}`,
      color: danger ? '#ff6b6b' : active ? '#7ec8a0' : '#c0c0c0',
...
export function Box({
  ...
  return (
    <div style={{
      ...
      borderRadius: 3, padding: '10px 14px', background: '#0f0f0f', minWidth: 160,
    }}>
