import { Await, Link, createRoute, notFound, redirect } from '@tanstack/react-router'
...
export const postDetailRoute = createRoute({
  ...
  notFoundComponent: () => (
    <div className="page-wrap px-4 pt-6 flex flex-col gap-3">
      <p className="text-(--sea-ink-soft)">Post not found.</p>
      <Link to="/dashboard/posts" className="text-(--lagoon-deep) hover:underline">
        ← Back to posts
      </Link>
    </div>
  ),
...
function PostDetailComponent() {
  const { post, relatedPosts } = postDetailRoute.useLoaderData()
  const { postId } = postDetailRoute.useParams()

  return (
    <div className="page-wrap px-4 pb-8 pt-6 flex flex-col gap-4">
      <Link to="/dashboard/posts" className="text-sm text-(--lagoon-deep) hover:underline">
        ← Back to posts
      </Link>
      <p className="text-xs text-(--sea-ink-soft)">
        useParams → postId: <strong>{postId}</strong>
      </p>
      <h1 className="text-2xl font-semibold">{post.title}</h1>
      <p className="text-(--sea-ink-soft)">{post.body}</p>
